...
def fitter_list():
    ...
    CONST_nmr_group_description = (
        "Fit your nuclear magnetic resonance (NMR) "
        "titration data to one of the binding models below."
    )
    CONST_uv_group_description = (
        "Fit your ultraviolet-visible (UV or UV-Vis) "
        "spectroscopic titration data to one of the binding models below. "
        "These models also work for flurescence titration if the free guest "
        'is "silent" (non-fluorescent) and dynamic quenching is absent.'
    )
    CONST_save_data_group_description = (
        "The below options allow you to save, "
...
def options(fitter, data_id=None, params=None, dilute=False):
    ...
    flavour_none = {"name": "None (Full)", "key": "none"}
    flavour_noncoop = {
        "name": "Non-cooperative",
        "key": "noncoop",
        "exclude_params": ["k2"],
    }
    flavour_add = {"name": "Additive", "key": "add"}
    flavour_stat = {
        "name": "Statistical",
        "key": "stat",
        "exclude_params": ["k2"],
    ...
    if not no_fit:
        fit = {
            ...
            "qof": {
                ...
                "cov_total": helpers.cov(
                    data["data"]["y"], residuals, total=True
                ),
            },
            "time": time,
            "options": {
...
def save(fit_id):
    response = {"fit_id": fit_id}
    return response


def export(url):
    response = {"export_url": url}
    return response
